    ...
    <div class="mall-directory-main">
        ...
        <div class="stores-list-section">
            <div class="stores-list" id="storesList">
                <?php
                // Only the first batch renders server-side; directory.js
                // renders the rest from #storesData on demand (initial view,
                // "load more" on scroll, and search/filter results) so 800+
                // store cards + logo images aren't all thrown into the DOM
                // and fetched on every page load.
                $initial_batch = array_slice($stores_data, 0, MALL_DIR_INITIAL_BATCH);
                foreach ($initial_batch as $store):
                    include MALL_DIR_PLUGIN_DIR . 'frontend/views/store-item.php';
                endforeach;
                ?>
                <div id="storesListSentinel" class="stores-list-sentinel" aria-hidden="true"></div>
            </div>
